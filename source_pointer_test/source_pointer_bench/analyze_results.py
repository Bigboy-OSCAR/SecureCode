...
def main() -> None:
    parser = argparse.ArgumentParser(description="Summarize source pointer benchmark JSONL results.")
    ...
    baseline = by_mode.get("full_source")
    if baseline:
        valid_baseline = [row for row in baseline if row.get("prompt_tokens") is not None or row.get("returncode") == 0]
        base_group = valid_baseline or baseline
        base_chars = mean(row["prompt_chars"] for row in base_group)
        base_wall = mean(row["wall_seconds"] for row in base_group)
        print()
        for mode in ["pointer_oracle", "pointer_select"]:
            group = by_mode.get(mode)
            if not group:
                continue
            chars = mean(row["prompt_chars"] for row in group)
            wall = mean(row["wall_seconds"] for row in group)
            print(f"{mode}_prompt_char_reduction_vs_full_source={(1 - chars / base_chars) * 100:.1f}%")
            if base_wall:
                print(f"{mode}_wall_time_change_vs_full_source={(1 - wall / base_wall) * 100:.1f}%")
